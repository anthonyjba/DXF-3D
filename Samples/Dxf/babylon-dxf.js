    ...
    function createScene(data, engine) {
      ...
      var drawEntity = function (entity, data) {
        if (entity.type === 'CIRCLE' || entity.type === 'ARC') {
          ...
          //console.log("line:" + entity.layer);
          if(entity.layer.indexOf('PS') > -1) 
              drawPolyLine(entity, data);
          else
            drawLine(entity, data);
        } else if (entity.type === 'POLYLINE') {
          //console.log("POLY:" + entity.layer);
          if(entity.layer.indexOf("-LF") > 0) {
            drawLine(entity, data); }
          else {
            drawPolyLine(entity, data); }
        } else if (entity.type === 'TEXT') {
          if(entity.layer.indexOf("-AU") > 0) {
      ...
      function drawPolyLine(entity, data) {
        if (entity.layer.indexOf("-LT") > -1 || entity.layer.indexOf("-LA") > -1) { return false; }
        ...
        // set material
        if (entity.layer.indexOf("-LP") > -1) {
          if(level != 1){
            var lines = BABYLON.Mesh.CreateLines("Linesbottom", path1, scene);
            lines.color = new BABYLON.Color3.Gray();
          }
        
          var polygonLP = BABYLON.Mesh.CreateRibbon("w-LP", [path1, path2], false, false, 20, scene);
          polygonLP.material = mat;
        }
        else {
            ...
            materialLI.diffuseTexture.uScale = 3;
            materialLI.diffuseTexture.vScale = 3;
            //materialLI.diffuseColor = new BABYLON.Color3(0.3, 0.3, 0.3);
            //materialLI.backFaceCulling = false;
            //materialLI.wireframe = true;
            materialLI.alpha = 0.6;

            //var polygon = BABYLON.Mesh.CreateRibbon("w-LI", [path1, path2], false, true, 0, scene, false, BABYLON.Mesh.BACKSIDE);
            //var polygon = BABYLON.Mesh.CreateRibbon("w-LI", [path1, path2], false, true, 0, scene, false, BABYLON.Mesh.BACKSIDE);
            var polygonLI = BABYLON.Mesh.CreateRibbon("poly", [path1, path2], true, false, 0, scene, false, BABYLON.Mesh.BACKSIDE);
            polygonLI.material = materialLI;
      ...
      var onlyLayersPS = data.entities.filter(function (e) {
        ...
        //return e.layer.substr(0, 4) === "PS05"
      })
